...
const Dashboard = () => {
  ...
  const cardData = [
    ...
    },
    {
      ...
      value: `₹ ${thisMonth?.credit?.toLocaleString() ?? 0}`,
      icon: TrendingUp,
      accent: 'from-emerald-500 to-neonBlue',
    },
    {
      label: 'This Month Debit',
      value: `₹ ${thisMonth?.debit?.toLocaleString() ?? 0}`,
      icon: CreditCard,
      accent: 'from-rose-500 to-neonPurple',
    },
    {
      label: 'Savings Potential',
      value: `₹ ${thisMonth?.savings?.toLocaleString() ?? 0}`,
      icon: PiggyBank,
      accent: 'from-amber-400 to-pink-500',
    },
    {
      label: 'Investment Gains',
      value: `₹ ${investmentSummary?.totalGain?.toLocaleString() ?? 0}`,
      description: `Invested ₹ ${investmentSummary?.totalInvested?.toLocaleString() ?? 0}`,
      icon: LineChart,
      accent:
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="glass-card p-5 space-y-4 min-w-0">
          ...
          <div className="text-sm text-slate-500">
            EMI pending: ₹ {totals?.emiPending?.toLocaleString() ?? 0}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-6">
          <div className="glass-card p-6 w-full max-w-md relative">
            ...
            <IncomeForm
              ...
              onSubmit={(payload) => actions.addIncome(payload)}
            />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-40 p-6">
          <div className="glass-card p-6 w-full max-w-lg space-y-4 relative">
            ...
            <p className="text-sm text-slate-500">
              {`Stay on track! ${reminders?.length ?? 0} EMI${
                (reminders?.length ?? 0) > 1 ? 's' : ''
              } due this month.`}
            </p>
